...
export const SimilarMovies = (props: MovieId) => {
    const { movieId } = props
    ...
    useEffect(() => {
        const fetchSimilar = async (page = 1) => {
            setLoading(true);

            try {
                const fetchSimilarMovie = await getSimilarMovie(page, movieId);
                setMovies(fetchSimilarMovie.results);
                console.log(fetchSimilarMovie);
    ...
    return (
        <div className="max-w-7xl mx-20 flex flex-col items-center">
            <div className="flex justify-between w-full">
                <p className="#09090B text-2xl font-semibold">More like this</p>
                <Link href={`/category/${""}`}>
